	...
	<div class="col-md-12">
		...
		<div class="card card-info">
			<div class="card-header">
				<h3 class="card-title"><?=lang('T01Projects.projectList') ?></h3>
			...
			<div class="card-body">
				<?= view('Themes/_commonPartialsBs/_alertBoxes'); ?>
					...
					<table id="tableOfProject" class="table table-striped table-hover using-exportable-data-table" style="width: 100%;">
						<thead>
							<tr>
								<th class="text-nowrap"><?= lang('Basic.global.Action') ?></th>
								<th><?= lang('T01Projects.nama') ?></th>
								<th class="text-nowrap"><?= lang('Basic.global.Action') ?></th>
							</tr>
						</thead>
						<tbody>
						<?php foreach ($projectList as $item ) : ?>
							<tr>
								<td class="align-middle text-center text-nowrap">
									<?=anchor(route_to('editProject', $item->id), '<i class="fas fa-pencil-alt"></i>', ['class'=>'btn btn-sm btn-warning btn-edit mr-1',  'data-id'=>$item->id,]); ?> 
									<?=anchor('#confirm2delete', '<i class="fas fa-trash"></i>', ['class'=>'btn btn-sm btn-danger btn-delete ml-1', 'data-href'=>route_to('deleteProject', $item->id)]); ?>
								</td>
			...
			<div class="card-footer">
				<?=anchor(route_to('newProject'), lang('Basic.global.addNew').' '.lang('T01Projects.project'), ['class'=>'btn btn-primary float-right']); ?>
			</div><!--//.card-footer -->
		</div><!--//.card -->
	</div><!--//.col -->
</div><!--//.row -->
...
<?=$this->section('additionalInlineJs') ?>
    ...
    $(document).on('click', '.btn-delete', function(e) {
            ...
            Swal.fire({
                title: "<?= lang('Basic.global.sweet.sureToDeleteTitle', [lang('T01Projects.project')]) ?>",
